...
### cgroups
> `cgroups`是linux内核提供的一种机制, 根据需求把一系列系统任务及其自任务整合到按资源划分等级的不同组内, 为系统资源管理提供同一的框架.
#### 4个特点
1. `cgroups`的API以伪文件系统方式实现. 用户态的程序通过文件操作实现对`cgroups`的组织管理
2. `cgroups`的组织管理操作单元可以细粒度到线程级别. 用户可以创建销毁`cgroup`, 实现对资源的分配和管理
3. 所有的资源管理的功能都是以子系统的形式实现, 接口统一
...
#### cgroups, 任务, 子系统, 层级四者的关系
1. 同一层级可以附加一个或者多个子系统
2. 一个子系统可以附加到多个层级, 当且仅当目标层级只有一个子系统时
3. 每次构建新的层级, 该系统上的所有任务都默认加入到新建层级的初始化`cgroups`,  称为`root cgroup`
4. 任务在`fork, clone`自身创建子任务默认与原任务在同一个`cgroup`中.

##### cgroups的本质, 是给任务挂上钩子, 当任务运行的过程设计某类资源, 就回出发钩子进行系统的检测.